...
- `IN2PETAADE_WEB_ROUTER_URL`: defaults to `https://app.t3.codes`.
- `IN2PETAADE_WEB_LATEST_DOMAIN`: defaults to `latest.app.t3.codes`.
- `IN2PETAADE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.t3.codes`.
...
- Repository slug source:
  - `IN2PETAADE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `IN2PETAADE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
