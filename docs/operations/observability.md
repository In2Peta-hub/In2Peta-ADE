...
In2Peta ADE has one server-side observability model:
...
```bash
export IN2PETAADE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export IN2PETAADE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export IN2PETAADE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export IN2PETAADE_TRACE_MIN_LEVEL=Info
export IN2PETAADE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `IN2PETAADE_OTLP_*`.

macOS app bundle example:

```bash
IN2PETAADE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
IN2PETAADE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
IN2PETAADE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/In2Peta ADE.app/Contents/MacOS/In2Peta ADE"
```
...
```bash
tail -f "$IN2PETAADE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$IN2PETAADE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$IN2PETAADE_HOME/userdata/logs/server.trace.ndjson"
```
...
- `IN2PETAADE_OTLP_TRACES_URL` was not set
...
- `IN2PETAADE_TRACE_FILE`: override trace file path
- `IN2PETAADE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `IN2PETAADE_TRACE_MAX_FILES`: rotated file count, default `10`
- `IN2PETAADE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `IN2PETAADE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `IN2PETAADE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `IN2PETAADE_OTLP_TRACES_URL`: OTLP trace endpoint
- `IN2PETAADE_OTLP_METRICS_URL`: OTLP metric endpoint
- `IN2PETAADE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `IN2PETAADE_OTLP_SERVICE_NAME`: service name, default `t3-server`
