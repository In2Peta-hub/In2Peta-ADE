...
This guide is for people who want to use more than one Claude setup in In2Peta ADE.
...
In In2Peta ADE Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means In2Peta ADE uses your normal home directory.
...
Then add another Claude provider in In2Peta ADE:
...
In2Peta ADE only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so In2Peta ADE keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in In2Peta ADE Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. In2Peta ADE stores the value as a server secret and does not
...
In2Peta ADE does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in In2Peta ADE.
