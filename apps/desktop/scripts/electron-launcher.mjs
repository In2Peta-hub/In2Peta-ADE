// This file mostly exists because we want dev mode to say "In2Peta ADE (Dev)" instead of "electron"
...
const devBundleIdSuffix = basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "In2Peta ADE (Dev)" : "In2Peta ADE (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.in2peta.ade.dev.${devBundleIdSuffix || "local"}`
  : "com.in2peta.ade";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["in2petaade-dev"] : ["in2petaade"];
...
function resolveDevelopmentProtocolCallbackPort() {
  const configuredPort = Number.parseInt(process.env.IN2PETAADE_PORT ?? "", 10);
...
function writeDevelopmentLauncherScript(targetBinaryPath, electronBinaryPath) {
  ...
  const envEntries = [
    ...
    ["IN2PETAADE_PORT", process.env.IN2PETAADE_PORT],
    ["IN2PETAADE_HOME", process.env.IN2PETAADE_HOME],
    ["IN2PETAADE_COMMIT_HASH", process.env.IN2PETAADE_COMMIT_HASH],
    ["IN2PETAADE_OTLP_TRACES_URL", process.env.IN2PETAADE_OTLP_TRACES_URL],
    ["IN2PETAADE_OTLP_EXPORT_INTERVAL_MS", process.env.IN2PETAADE_OTLP_EXPORT_INTERVAL_MS],
    ["IN2PETAADE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
    ["IN2PETAADE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED", "1"],
    ["IN2PETAADE_DESKTOP_PROTOCOL_CALLBACK_URL", protocolCallbackUrl],
  ...
  writeFileSync(
    ...
    [
      ...
      'for arg in "$@"; do',
      '  case "$arg" in',
      "    in2petaade-dev://auth/callback*)",
      '      if [ -n "$IN2PETAADE_DESKTOP_PROTOCOL_CALLBACK_URL" ]; then',
      '        /usr/bin/curl -fsS --max-time 2 -X POST --data-binary "$arg" "$IN2PETAADE_DESKTOP_PROTOCOL_CALLBACK_URL" >/dev/null 2>&1 && exit 0',
      "      fi",
      "      ;;",
      "  esac",
      "done",
      `exec ${shellSingleQuote(electronBinaryPath)} --in2petaade-dev-root=${shellSingleQuote(desktopDir)} ${shellSingleQuote(mainEntryPath)} "$@"`,
      "",
    ].join("\n"),
  );
