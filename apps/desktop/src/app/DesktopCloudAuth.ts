...
function resolveConfiguredProtocolClient(): {
  ...
  const path = process.env.IN2PETAADE_DESKTOP_PROTOCOL_CLIENT_PATH?.trim();
  if (!path) return null;

  return {
    path,
    args: (process.env.IN2PETAADE_DESKTOP_PROTOCOL_CLIENT_ARGS ?? "")
      .split("\n")
      .map((arg) => arg.trim())
...
function isProtocolRegistrationManagedExternally(): boolean {
  return process.env.IN2PETAADE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED?.trim() === "1";
}

function resolveProtocolCallbackForwardUrl(): URL | null {
  const rawUrl = process.env.IN2PETAADE_DESKTOP_PROTOCOL_CALLBACK_URL?.trim();
