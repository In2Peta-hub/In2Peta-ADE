...
const LINUX_WM_CLASS = isDevelopment ? "in2petaade-dev" : "in2petaade";
const USER_DATA_DIR_NAME = isDevelopment ? "in2petaade-dev" : "in2petaade";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "In2Peta ADE (Dev)" : "In2Peta ADE";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("In2Peta ADE failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
