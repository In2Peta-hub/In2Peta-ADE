...
const originalClerkPublishableKey = process.env.IN2PETAADE_CLERK_PUBLISHABLE_KEY;
...
describe("Desktop cloud auth IPC", () => {
  afterEach(() => {
    ...
    if (originalClerkPublishableKey === undefined) {
      delete process.env.IN2PETAADE_CLERK_PUBLISHABLE_KEY;
    } else {
      process.env.IN2PETAADE_CLERK_PUBLISHABLE_KEY = originalClerkPublishableKey;
    }
  });
  ...
  it.effect(
    ...
    () => {
      process.env.IN2PETAADE_CLERK_PUBLISHABLE_KEY = clerkPublishableKey("clerk.t3.codes");
