    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          IN2PETAADE_PORT: "9999",
          IN2PETAADE_MODE: "desktop",
          IN2PETAADE_DESKTOP_LAN_HOST: "192.168.1.50",
  ...
  it.effect("resolves backend start config with a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.IN2PETAADE_PORT);
        assert.isUndefined(first.env.IN2PETAADE_MODE);
        assert.isUndefined(first.env.IN2PETAADE_DESKTOP_LAN_HOST);
