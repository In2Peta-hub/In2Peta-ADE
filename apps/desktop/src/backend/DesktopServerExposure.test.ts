...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return makeDesktopEnvironmentLayer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ IN2PETAADE_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { IN2PETAADE_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        IN2PETAADE_DESKTOP_LAN_HOST: "10.0.0.7",
        IN2PETAADE_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
  ...
  it.effect("advertises loopback, LAN, and configured manual endpoints from runtime state", () =>
    withHarness(
      ...
      {
        IN2PETAADE_DESKTOP_HTTPS_ENDPOINTS:
