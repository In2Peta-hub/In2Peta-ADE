    ...
    try {
      await vi.waitFor(
        () => {
          ...
          expect(openRequest?.env?.IN2PETAADE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: In2Peta ADE Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
