  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "in2petaade/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "in2petaade/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "in2petaade/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "in2petaade/tmp-working",
          newBranch: "in2petaade/feat/session",
        });

        expect(renamed.branch).toBe("in2petaade/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "in2petaade/feat/session")).toBe(
          true,
        );
        expect(
          branches.branches.some((branch) => branch.name === "in2petaade/feat/session-1"),
        ).toBe(true);
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("in2petaade/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "in2petaade/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "in2petaade/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "in2petaade/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "in2petaade/tmp-working" });
        ...
        expect(renamed.branch).toBe("in2petaade/feat/session-2");
      }),
    );
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, [
            "checkout",
            "-b",
            "in2petaade/pr-488/statemachine",
            "--track",
