...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("IN2PETAADE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("IN2PETAADE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("IN2PETAADE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("IN2PETAADE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set IN2PETAADE_BITBUCKET_EMAIL and IN2PETAADE_BITBUCKET_API_TOKEN, or IN2PETAADE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
