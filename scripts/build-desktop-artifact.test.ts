...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "In2Peta ADE");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "In2Peta ADE (Nightly)");
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.in2peta.ade</string>");
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                IN2PETAADE_DESKTOP_SKIP_BUILD: "true",
                IN2PETAADE_DESKTOP_KEEP_STAGE: "true",
                IN2PETAADE_DESKTOP_SIGNED: "true",
                IN2PETAADE_DESKTOP_VERBOSE: "true",
                IN2PETAADE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
