...
const DESKTOP_APP_ID = "com.in2peta.ade";
...
export function createAsarUnpackPatterns(platform: typeof BuildPlatform.Type): readonly string[] {
  ...
  // them, so unpacking just the bundle + node-pty is not enough. Keep this
  // broad unpacking Windows-only so macOS/Linux release packaging does not
  // duplicate the full dependency tree into app.asar.unpacked.
  return [...DESKTOP_ASAR_UNPACK, ...WINDOWS_WSL_ASAR_UNPACK];
}
