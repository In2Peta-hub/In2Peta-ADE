...
export const DEFAULT_IN2PETAADE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".in2petaade"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("IN2PETAADE_PORT_OFFSET"),
  devInstance: optionalStringConfig("IN2PETAADE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid IN2PETAADE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `IN2PETAADE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric IN2PETAADE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed IN2PETAADE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_IN2PETAADE_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      IN2PETAADE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.IN2PETAADE_PORT = String(serverPort);
      ...
      delete output.IN2PETAADE_MODE;
      delete output.IN2PETAADE_NO_BROWSER;
      delete output.IN2PETAADE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.IN2PETAADE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.IN2PETAADE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.IN2PETAADE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.IN2PETAADE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.IN2PETAADE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.IN2PETAADE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.IN2PETAADE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.IN2PETAADE_MODE = "web";
      delete output.IN2PETAADE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read IN2PETAADE_PORT_OFFSET/IN2PETAADE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.IN2PETAADE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.IN2PETAADE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Base directory for all In2Peta ADE data (equivalent to IN2PETAADE_HOME).",
    ),
    Flag.withFallbackConfig(optionalStringConfig("IN2PETAADE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to IN2PETAADE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("IN2PETAADE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to IN2PETAADE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("IN2PETAADE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to IN2PETAADE_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("IN2PETAADE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to IN2PETAADE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("IN2PETAADE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to IN2PETAADE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("IN2PETAADE_PORT")),
  ),
