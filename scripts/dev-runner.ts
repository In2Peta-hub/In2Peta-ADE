...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("IN2PETAADE_PORT_OFFSET"),
...
> {
    ...
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "IN2PETAADE_PORT_OFFSET",
  ...
  if (/^\d+$/.test(seed)) {
    return Effect.succeed({
      offset: Number(seed),
      source: `numeric IN2PETAADE_DEV_INSTANCE=${seed}`,
    });
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return Effect.succeed({ offset, source: `hashed IN2PETAADE_DEV_INSTANCE=${seed}` });
}
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["IN2PETAADE_PORT_OFFSET", "IN2PETAADE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
